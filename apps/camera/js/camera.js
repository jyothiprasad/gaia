...
var DUAL_BUTTON = constants.DUAL_BUTTON;
var tFocusCoord ={x:0,y:0};
var touchBool = false;
...
function Camera(options) {
  ...
  this.set('dualbutton', DUAL_BUTTON);
...
Camera.prototype.configureCamera = function(mozCamera) {
  ...
  this.configurePictureSize(mozCamera, this.pictureSize);
  this.configureFocus(capabilities.focusModes);
  this.configureFlash(capabilities.flashModes);
  this.configureVideoProfile(capabilities, this.videoProfile, done());


  //set Default White balance mode Auto  camera.whiteBalanceMode
  this.setWhiteBalanceMode("auto");
  //set Default scene mode
  this.setSceneMode("auto");
  //set Default iso auto
  this.setISO('auto');
...
Camera.prototype.configurePictureSize = function(mozCamera, imageSize) {
  var capabilities = mozCamera.capabilities;
  var pictureSizes = capabilities.pictureSizes;
  var thumbnailSizes = capabilities.thumbnailSizes;

  // Store picked pictureSize
  this.pictureSize = this.pickPictureSize(pictureSizes, imageSize);
...
Camera.prototype.configureFlash = function(allModes) {
  ...
  /*if(cameraMode === 'video' && index == 1) {
    if(this.mozCamera.getParameter("luminance-condition") == "low")
      index = 2;
    else
      index = 0;
  }*/
...
Camera.prototype.configureVideoProfile = function(capabilities, videoSize, done) {
  var self = this;
...
Camera.prototype.configurePreviewSize = function(photoPreviewSizes,
  ...
  if (this.get('mode') === 'video' && this.videoPreviewSize.width && this.videoPreviewSize.height) {
    this.previewSize = this.videoPreviewSize;
    return;
  } else if (this.get('mode') === 'photo' && this.photoPreviewSize.width && this.photoPreviewSize.height) {
...
Camera.prototype.pickVideoProfile = function(profiles, preferredSizes, videosize) {
  ...
  var profileName;
  var profile;
  if(!targetFileSize && videosize.width) {
    return videosize;
  }

  if (videosize) {
   if (videosize in profiles) {
      matchedProfileName = videosize;
    }
  } else  if (preferredSizes) {
...
Camera.prototype.pickPictureSize = function(pictureSizes, imageSize) {
  ...
  // change in picksize
  if (imageSize && imageSize.width*imageSize.height <= maxRes && !targetSize) {
    return imageSize;
  }
...
Camera.prototype.prepareTakePicture = function(done) {
  ...
  if (!this.autoFocus.auto || this.get('recording')) {
    done();
    return;
  }
 ...
 * @return {String}
 */
Camera.prototype.toggleBode = function() {

    console.log('CCC this.previewSize.width: '+ this.previewSize.width);
    if ( this.previewSize.width != 1280) {
      this.previewSize.width = 1280;
      this.previewSize.height = 720;
    } else {
      this.previewSize.width = 960;
      this.previewSize.height = 720;
    }
    this.photoPreviewSize = this.previewSize;
    this.emit('selfish');
};
 ...
 * @return {String}
 */
Camera.prototype.changeVideoMode = function() {
  var mode = 'video';
  this.set('mode', mode);
  this.previewSize = this.videoPreviewSize;
  this.configureFlash(this.flash.all);
...
Camera.prototype.setZoom = function(scale) {
  ...
  //console.log("scale : " + scale + "   _cameraObj.zoom: " + this.mozCamera.zoom);
};
...
Camera.prototype.setCaptureTimer = function(timer){
  this._captuerTimer = timer;
};

Camera.prototype.getCaptureTimer = function(timer){
  return this._captuerTimer;
};

Camera.prototype.setImagesize = function(imageSize, previewSize) {
  this.photoPreviewSize = this.previewSize = previewSize;
  //this.emit('previewUpdated');
  //debug('previewUpdated');
  this.emit('changeImagePreview');
  //this.pictureSize = imageSize;
  this.configurePictureSize(this.mozCamera, imageSize);

};

Camera.prototype.setImagePreviewSize = function(previewSize) {
  this.photoPreviewSize = previewSize;
 // this.emit('changeImagePreview');
};

Camera.prototype.setVideoProfile = function(videoSize) {
  this.configureVideoProfile(this.mozCamera.capabilities, videoSize,null);
};
Camera.prototype.setVideoPreviewSize = function(videoSize, videoPreviewSize) {
  var profile = {
    profile: videoSize,
    rotation: 0,
    width: videoPreviewSize.width,
    height: videoPreviewSize.height
  };
...
Camera.prototype.setVideoProfile = function(videoSize, videoPreviewSize) {
  this.videoPreviewSize = videoPreviewSize;
...
Camera.prototype.setISO = function(value){
...
Camera.prototype.setWhiteBalanceMode = function(value){
...
//Focus
Camera.prototype.setTouchFocusMode = function(x,y){
  var self = this;
 ...
 // var scalePts = null; //this.scalePoints(x,y);
  ...
  var sh = this.previewSize.height/deviceIndependentViewportSize.height;
   console.log('CCC sw: '+sw);
  console.log('CCC sh: '+sh);
  var px = x * sh;
  var py = y * sw;
  ...
  //{bottom:208,left:192,right:208,top:192,weight:1};
  //this.mozCamera.meteringAreas = '(208,192,208,192,1)';//'(left,top,right,bottom,1)';

  this.mozCamera.focusMode = "continuous-picture";
/*
  console.log('CCC _left '+_left);
  console.log('CCC _top '+_top);
  console.log('CCC _right '+_right);
  console.log('CCC _bottom '+_bottom);

  this.mozCamera.enableAutoFocusMove(onAutoFocusMove);
  this.set('focus', 'focusing');

 // this.emit('touchFocusing');
  function onAutoFocusMove(success) {
    var bool1 = success;
    if(bool1 === true) {
    //  self.emit('touchFocused');
      self.set('focus', 'focused');
      //self.mozCamera.disableAutoFocusMove();
      self.mozCamera.focusMode = "none";
    } else {
     // self.set('focus', 'fail');
     // setTimeout(function() {
        self.set('focus', 'none');
    ...
    //touchBool = false;
    console.log("CCC AutoFocusMove Value  = "+ bool1);
...
Camera.prototype.getTouchFocusPoints = function(){
  return tFocusCoord;
};

Camera.prototype.setTouchFlag = function(){
  touchBool = true;
};

Camera.prototype.getTouchFlag = function(){
  return touchBool;
};
